  ...
  loginEmployee: async (req, res) => {
    try {
      ...
      if (activation == true) {
        ...
        //Using token for login
        const tokenLoginEmployee = jwt.sign({ id: user._id, role: user.role, firstLogin: user.firstLogin }, `${process.env.JWT_SECRET}`);

        res.cookie("tokenLoginEmployee", tokenLoginEmployee);
        //return res.json({ message: tokenLogin })
